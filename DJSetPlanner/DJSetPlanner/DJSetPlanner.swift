...
private let kDeepSeekAPIKey = "sk-a10d8aa6913d43b2acb366760d8f3aa6"
// Point this at a deployed NeteaseCloudMusicApi-compatible service.
private let kNeteaseAPIBaseURL = "https://ncm.nekogan.com"

// MARK: - Track Recommendation

struct RecommendedTrack: Identifiable {
    let id: String
    let name: String
    let artist: String
...
enum BPMAnalysisStage: Equatable {
    case idle
    case searching
    case downloading(slot: Int)
    case analyzing(slot: Int)
    ...
    func label(isChinese: Bool) -> String {
        ...
        case .idle:
            return ""
        case .searching:
            return isChinese ? "正在网易云搜索候选曲目..." : "Searching NetEase candidates..."
        case .downloading(let slot):
            return isChinese ? "正在下载第 \(slot) 段候选音频..." : "Downloading audio for slot \(slot)..."
        case .analyzing(let slot):
            return isChinese ? "正在分析第 \(slot) 段 BPM..." : "Analyzing BPM for slot \(slot)..."
        case .finished(let count):
            return isChinese ? "BPM 分析完成，匹配 \(count) 首曲目" : "BPM analysis complete, matched \(count) tracks"
        }
    }
}

// MARK: - NetEase Search + Local BPM Analysis

actor NeteaseMusicService {
    static let shared = NeteaseMusicService()
    private init() {}

    private var usedTrackIds: Set<String> = []

    func resetSession() { usedTrackIds = [] }

    private func isGenreLabel(_ trackName: String, query: String) -> Bool {
        let name = trackName.lowercased().trimmingCharacters(in: .whitespaces)
        let q    = query.lowercased().trimmingCharacters(in: .whitespaces)
        if name == q { return true }
        let stopWords: Set<String> = ["and", "the", "a", "of", "in", "for", "de", "la", "le"]
        let qWords = q.split(separator: " ").map(String.init).filter { $0.count > 2 && !stopWords.contains($0) }
        guard !qWords.isEmpty else { return false }
        let matchCount = qWords.filter { name.contains($0) }.count
        return Double(matchCount) / Double(qWords.count) >= 0.75
    }
    ...
    func search(
        ...
        genreTags: String,
        slot: Int,
        progress: (@MainActor (BPMAnalysisStage) -> Void)? = nil
        ...
        for query in queries {
            let candidates = try await searchCandidates(query: query, offset: min((slot - 1) * 5, 60))

            var tracks: [RecommendedTrack] = []
            for candidate in candidates {
                guard !isGenreLabel(candidate.name, query: primaryTag) else { continue }
                guard !usedTrackIds.contains(candidate.id) else { continue }
                guard let songUrl = try await resolveSongURL(id: candidate.id) else { continue }

                await progress?(.downloading(slot: slot))
                guard let localAudioURL = try? await downloadAudio(from: songUrl) else { continue }
                await progress?(.analyzing(slot: slot))

                let estimate = try? BPMAnalyzer.estimateBPM(from: localAudioURL, targetBPM: bpm)
                try? FileManager.default.removeItem(at: localAudioURL)
                guard let estimate else { continue }

                usedTrackIds.insert(candidate.id)
                tracks.append(RecommendedTrack(
                    id: candidate.id,
                    name: candidate.name,
                    artist: candidate.artist,
                    sourceUrl: "https://music.163.com/#/song?id=\(candidate.id)",
                    measuredBPM: estimate.bpm,
                    bpmConfidence: estimate.confidence
                ))
                if tracks.count == 3 { break }
            }

            print("[NetEase+BPM] slot \(slot) query '\(query)' -> \(tracks.count) tracks")
            if !tracks.isEmpty { return tracks }
        }
        return []
    }

    private func searchCandidates(query: String, offset: Int) async throws -> [NeteaseCandidate] {
        let encoded = query.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? query
        guard let url = URL(string: "\(kNeteaseAPIBaseURL)/cloudsearch?keywords=\(encoded)&limit=12&offset=\(offset)") else {
            return []
        }
        ...
        return songs.compactMap { item in
            ...
            return NeteaseCandidate(id: id, name: name, artist: artist.isEmpty ? "Unknown Artist" : artist)
        }
    }

    private func resolveSongURL(id: String) async throws -> URL? {
        guard let url = URL(string: "\(kNeteaseAPIBaseURL)/song/url/v1?id=\(id)&level=standard") else {
            return nil
        }
        ...
        guard (response as? HTTPURLResponse)?.statusCode == 200,
              ...
              var rawURL = first["url"] as? String else { return nil }

        if rawURL.hasPrefix("http://") {
            rawURL = "https://" + String(rawURL.dropFirst("http://".count))
        }
        guard let songURL = URL(string: rawURL) else { return nil }
        return songURL
    }
...
private struct NeteaseCandidate {
    let id: String
    let name: String
    let artist: String
}

private struct BPMEstimate {
    let bpm: Int
    let confidence: Double
}

private enum BPMAnalyzer {
    static func estimateBPM(from url: URL, targetBPM: Int) throws -> BPMEstimate {
        let file = try AVAudioFile(forReading: url)
        let sampleRate = file.fileFormat.sampleRate
        let secondsToRead = min(45.0, Double(file.length) / sampleRate)
        let frameCount = AVAudioFrameCount(max(1, secondsToRead * sampleRate))
        ...
        let hop = max(512, Int(sampleRate / 43))
        var envelope: [Float] = []
        var index = 0
        while index < frames {
            let end = min(index + hop, frames)
            var sum: Float = 0
            for frame in index..<end {
                var mono: Float = 0
                for channel in 0..<channelCount {
                    mono += abs(channels[channel][frame])
                }
                sum += mono / Float(channelCount)
            }
            envelope.append(sum / Float(max(1, end - index)))
            index += hop
        }
        ...
        let normalized = envelope.map { max(0, $0 - mean) }
        let envelopeRate = sampleRate / Double(hop)
        let minBPM = max(60, targetBPM - 35)
        let maxBPM = min(210, targetBPM + 35)
        var bestBPM = targetBPM
        var bestScore: Float = 0
        var totalScore: Float = 0

        for bpm in minBPM...maxBPM {
            let lag = max(1, Int((60.0 / Double(bpm)) * envelopeRate))
            guard lag < normalized.count else { continue }
            var score: Float = 0
            for i in lag..<normalized.count {
        ...
        let confidence = totalScore > 0 ? min(1.0, Double(bestScore / totalScore) * 8.0) : 0
        return BPMEstimate(bpm: bestBPM, confidence: confidence)
    }
}
...
final class NotificationService {
    ...
    func notifyAnalysisFinished(count: Int, isChinese: Bool) {
        let content = UNMutableNotificationContent()
        content.title = isChinese ? "BPM 分析完成" : "BPM Analysis Complete"
        content.body = isChinese ? "已匹配 \(count) 首网易云曲目。" : "Matched \(count) NetEase tracks."
        content.sound = .default
        let request = UNNotificationRequest(identifier: "bpm-analysis-\(UUID().uuidString)", content: content, trigger: nil)
...
final class BackgroundExecutionCoordinator {
    ...
    func begin() {
        ...
        taskIdentifier = UIApplication.shared.beginBackgroundTask(withName: "BPMAnalysis") { [weak self] in
            Task { @MainActor in
                self?.end()
            }
        }
...
class DJSetViewModel: ObservableObject {
    ...
    @Published var recommendedTracks: [Int: [RecommendedTrack]] = [:]
    @Published var isAnalyzingBPM: Bool = false
    @Published var bpmAnalysisStage: BPMAnalysisStage = .idle

    private let backgroundExecution = BackgroundExecutionCoordinator()
...
"""
        ...
        // Fetch NetEase recommendations and estimate BPM from downloaded audio.
        if let plan = setPlan {
            isAnalyzingBPM = true
            bpmAnalysisStage = .searching
            backgroundExecution.begin()
            await NeteaseMusicService.shared.resetSession()
            let tags = genreTags
            var results: [Int: [RecommendedTrack]] = [:]
            for t in plan.tracks {
                bpmAnalysisStage = .searching
                let tracks = (try? await NeteaseMusicService.shared.search(
                    bpm: t.bpm,
                    energy: t.energy,
                    genreTags: tags,
                    slot: t.slot,
                    progress: { [weak self] stage in
                        self?.bpmAnalysisStage = stage
                    }
                )) ?? []
                results[t.slot] = tracks
                recommendedTracks = results
            }
            let count = results.values.reduce(0) { $0 + $1.count }
            bpmAnalysisStage = .finished(count: count)
            isAnalyzingBPM = false
            backgroundExecution.end()
            NotificationService.shared.notifyAnalysisFinished(count: count, isChinese: isChinese)
        }
    }
    ...
    func plainTextExport() -> String {
        ...
        for t in plan.tracks {
            ...
            lines.append("    \(t.vibe)")
            if let recs = recommendedTracks[t.slot], !recs.isEmpty {
                lines.append(isChinese ? "    推荐曲目:" : "    Recommended Tracks:")
                for rec in recs {
                    let bpmText = rec.measuredBPM.map { " | \($0) BPM" } ?? ""
                    lines.append("    • \(rec.name) - \(rec.artist)\(bpmText)")
                }
            }
            lines.append("")
        }
...
/// Shared generators avoid the one-time Taptic Engine driver warmup cost
/// that would otherwise block the main thread on first interaction.
...
private func haptic(_ style: UIImpactFeedbackGenerator.FeedbackStyle = .light) {
    switch style {
    case .light:  HapticFeedback.light.impactOccurred()
    case .medium: HapticFeedback.medium.impactOccurred()
...
struct DJSetPlannerApp: App {
    init() {
        // Pre-warm the Taptic Engine so the first button tap / slider drag
        // does not block the main thread and cause a scroll freeze.
        HapticFeedback.light.prepare()
        HapticFeedback.medium.prepare()
...
struct ContentView: View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Slider(value: $vm.duration, in: 15...360, step: 5)
                    .tint(.djNeon)
                    .onChange(of: vm.duration) { hapticSelection() }
            }
                ...
                HStack(spacing: 12) {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Slider(value: $vm.bpmMin, in: 60...200, step: 1)
                                .tint(.djNeon)
                                .onChange(of: vm.bpmMin) { hapticSelection() }
                    }
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Slider(value: $vm.bpmMax, in: 60...200, step: 1)
                            .tint(.djNeon2)
                            .onChange(of: vm.bpmMax) { hapticSelection() }
                    }
                }
            }
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 8) {
                        ForEach(MusicGenre.all) { genre in
                            ...
                            Button { haptic(.light); vm.applyGenre(genre) } label: {
            ...
            .disabled(vm.isLoading || vm.isAnalyzingBPM)
            .opacity((vm.isLoading || vm.isAnalyzingBPM) ? 0.5 : 1)
        ...
        VStack(spacing: 12) {
            ...
            ForEach(plan.tracks) { track in
                TrackCardView(track: track, isChinese: vm.isChinese,
                              recommendedTracks: vm.recommendedTracks[track.slot] ?? [],
                              isAnalyzingBPM: vm.isAnalyzingBPM)
            }

            DJNotesView(notes: plan.djNotes, isChinese: vm.isChinese)

            if vm.isAnalyzingBPM || vm.bpmAnalysisStage != .idle {
                analysisStatusCard(vm.bpmAnalysisStage)
            }

            // Copy full setlist button
            if !vm.recommendedTracks.isEmpty {
                Button(action: {
                    let sections = plan.tracks.compactMap { slot -> String? in
                        guard let tracks = vm.recommendedTracks[slot.slot], !tracks.isEmpty else { return nil }
                        let header = "— Slot \(slot.slot) | \(slot.bpm) BPM | Energy \(slot.energy) —"
                        let lines = tracks.map { track in
                            let bpmText = track.measuredBPM.map { " | \($0) BPM" } ?? ""
                            return "\(track.name) - \(track.artist)\(bpmText)"
                        }.joined(separator: "\n")
                        return "\(header)\n\(lines)"
                    }
                    ...
                    Label(vm.isChinese ? "复制完整歌单" : "Copy Full Setlist",
                          systemImage: "doc.on.doc")
                        .font(.system(size: 14, weight: .semibold))
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 12)
                        .background(Color.djNeon.opacity(0.12))
                        .foregroundColor(.djNeon)
                        .overlay(
                            RoundedRectangle(cornerRadius: 10)
                                .stroke(Color.djNeon, lineWidth: 1)
    ...
    func analysisStatusCard(_ stage: BPMAnalysisStage) -> some View {
        HStack(spacing: 10) {
            if vm.isAnalyzingBPM {
                ProgressView()
                    .scaleEffect(0.8)
                    .tint(.djNeon)
            } else {
            ...
            Text(stage.label(isChinese: vm.isChinese))
                .font(.system(size: 12, weight: .semibold))
                .foregroundColor(.djMuted)
            Spacer()
        }
...
struct TrackCardView: View {
    let track: TrackSlot
    let isChinese: Bool
    let recommendedTracks: [RecommendedTrack]
    let isAnalyzingBPM: Bool
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack(spacing: 6) {
                    Text(isChinese ? "能量" : "ENERGY")
                        ...
                        .foregroundColor(.djMuted)
                        .kerning(1)
                    // overlay+scaleEffect avoids a GeometryReader layout pass
                    RoundedRectangle(cornerRadius: 2)
                        ...
                        .overlay(alignment: .leading) {
                            RoundedRectangle(cornerRadius: 2)
                                .fill(energyColor)
                                .scaleEffect(x: CGFloat(track.energy) / 10, anchor: .leading)
                        }
                    Text("\(track.energy)")
                ...
                // NetEase recommendations with locally estimated BPM.
                if isAnalyzingBPM && recommendedTracks.isEmpty {
                    HStack(spacing: 6) {
                        ProgressView().scaleEffect(0.7).tint(.djNeon)
                        Text(isChinese ? "正在搜索并分析 BPM..." : "Searching and analyzing BPM...")
                            .font(.system(size: 10))
                            .foregroundColor(.djMuted)
                    }
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        Text(isChinese ? "▶ 推荐曲目" : "▶ RECOMMENDED TRACKS")
                            ...
                            .foregroundColor(.djNeon)
                            .kerning(1)
                        ForEach(recommendedTracks) { rec in
                            Link(destination: URL(string: rec.sourceUrl)!) {
                                HStack(spacing: 8) {
                                ...
                                Spacer()
                                if let measuredBPM = rec.measuredBPM {
